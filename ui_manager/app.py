...
@login_required
def model_upload():
    print("got request model upload")
    if request.method == 'POST':
...
@login_required
def app_upload():
    print("in app upload request")
    if request.method == 'POST':
        print("in post request")
        if 'customFile' not in request.files:
            print("flash('No file part')")
            flash('No file part')
        ...
        if file and allowed_file(file.filename):
            ...
            if flag:
                ...
                with ZipFile(name, 'a') as zipf:
                    ...
                    print(os.getcwd())
                    print("sending to ")
                    print(f"{file.filename.split('.')[0]}/api.py")
...
def configuration_upload():
    ...
    return redirect(f"http://localhost:{scheduler_service_port}/")
