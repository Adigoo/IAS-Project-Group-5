...
def execute(portNumber):
    ...
    docker = DockerClient(compose_files=["./{serviceName}-dockerCompose.yaml".format(serviceName = constants.SERVICE_SENSOR_MANAGER)])
    docker.compose.build()
    docker.compose.up(detach=True)
    logging.debug('{serviceName} loaded successfully'.format(serviceName = constants.SERVICE_SENSOR_MANAGER))